...
router.post('/join', isNotLoggedIn, async (req, res, next) => {
   try {
      ...
      // 비밀번호 암호화
      const hash = await bcrypt.hash(password, 12) // 12: salt(해쉬 암호화를 진행시 추가되는 임의의 데이터 주로 10 ~ 12 정도의 값이 권장됨)
...
router.post('/login', isNotLoggedIn, async (req, res, next) => {
   // authenticate 함수는 localStrategy.js 에 작성한 인증과정을 실행한다. 그 과정에서 에러발생시 authError 객제에 값을 주고, 인증과정 성공시 user에는 인증과정에서 passport에 넘겨줬던 exUser값이 들어있다.
   passport.authenticate('local', (authError, user, info) => {
      if (authError) {
         // 로그인 인증 중 에러 발생시
         authError.status = 500
         authError.message = '인증 중 오류 발생'
         return next(authError) // 에러 미들웨어로 보냄
...
router.get('/logout', isLoggedIn, async (req, res, next) => {
   req.logout((logoutError) => {
      ...
      // 로그아웃 성공시 세션에 저장되어 있던 사용자 id는 삭제된다.
      // 로그아웃 성공시 response
      res.status(200).json({
         success: true,
