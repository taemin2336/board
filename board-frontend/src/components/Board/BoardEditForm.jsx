...
function BoardEditForm({ onBoardEdit, initialValues = {} }) {
   ...
   const [title, setTitle] = useState(initialValues.title) // 게시물 제목
   ...
   const handleSubmit = (e) => {
      ...
      // 수정시 이미지 파일을 바꾸지 않는 경우도 있으므로 이미지 파일 체크 사용X
      // if (!imgFile) {
      //    alert('이미지 파일을 추가하세요.')
      //    return
      // }
      ...
      formData.append('title', title) // 게시물 제목
   ...
   return (
      <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3 }} encType="multipart/form-data">
         ...
         {/* 게시물 내용 입력 필드 */}
         <TextField label="게시물 내용" variant="outlined" fullWidth multiline rows={2} value={title} onChange={(e) => setTitle(e.target.value)} sx={{ mt: 2 }} />
